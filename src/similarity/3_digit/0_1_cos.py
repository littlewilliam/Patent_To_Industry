...
ind = pd.read_csv('../../../res/Industry/3_digit/GMJJHY_OK_A_n_list.csv', dtype=str)
...
def read_ind_code():
    ...
    ind_code = pd.read_csv('../../../res/Industry/3_digit/GMJJHY_Comment.csv', dtype=str)
    l_ind_code = []
    l_ind = []
    ...
    for index, item in ind_code.iterrows():
        ...
        code = int(item['code'])
        # 此处为判断A	农、林、牧、渔业类目下的所有3位精度的国民经济行业类目,可调整
        if code <= 54:
...
new_data.to_csv('../../../out/3_digit/cos_A_A01.csv', index=False, encoding='gbk')
